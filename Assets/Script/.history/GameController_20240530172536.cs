...
public class GameController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isCountingDown)
        {
            ...
            if (displayTime >= 0)
            {
                text.GetComponent<TMP_Text>().text = displayTime.ToString();
            }
            else if (displayTime >= -1f)
            {
                text.GetComponent<TMP_Text>().text = "Ready";
            } 
            else if (displayTime >= -2f)
            {
                text.GetComponent<TMP_Text>().text = "Go";
            } 
            else
            {
                text.GetComponent<TMP_Text>().text = "";
                isCountingDown = false;
                sampleAnimation1.StartGame(true);
            }
        }
    }
