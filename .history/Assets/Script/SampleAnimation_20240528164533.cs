...
public class SampleAnimation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            this.animator.SetBool("umatobi", true);
        }
        else if (Input.GetKeyUp("u"))
        {
                this.animator.SetBool("umatobi", false);
        }
        ...
        {
            this.animator.SetBool("lie", true);
            this.animator.SetBool(key_ifRun, false);
            this.animator.SetBool(key_isForward, false);
            this.animator.SetBool(key_isJump, false);
            this.animator.SetBool(key_isWalkBackward, false);
            this.animator.SetFloat(key_Blend, 0f);
        }
        else if (Input.GetKeyUp("l"))
        {
            this.animator.SetBool("lie", false);
            this.animator.SetFloat(key_Blend, 0.5f);
        }
        
        // 倒下锁定速度
        if (this.animator.GetCurrentAnimatorStateInfo(0).shortNameHash.ToString() == "Lie")
        {
            speed = 0f;
        }
        else
        {
            if (this.animator.GetBool(key_ifRun))
            {
                speed = 10f;
            }
            else
